...
let BulkFindOp = class BulkFindOp extends decorators_1.ShellApiWithMongoClass {
    ...
    hint(hintDoc) {
        helpers_1.assertArgsDefinedType([hintDoc], [true], 'BulkFindOp.hint');
        this._hint = hintDoc;
        return this;
    }
    delete() {
    ...
    replaceOne(replacement) {
        ...
        helpers_1.assertArgsDefinedType([replacement], [true], 'BulkFindOp.replacement');
    ...
    updateOne(update) {
        this._parentBulk._batchCounts.nUpdateOps++;
        helpers_1.assertArgsDefinedType([update], [true], 'BulkFindOp.update');
        const op = { ...update };
        if (this._hint) {
            op.hint = this._hint;
        }
...
__decorate([
    decorators_1.returnType('BulkFindOp'),
...
let Bulk = class Bulk extends decorators_1.ShellApiWithMongoClass {
    ...
    find(query) {
        helpers_1.assertArgsDefinedType([query], [true], 'Bulk.find');
        return new BulkFindOp(this._serviceProviderBulkOp.find(query), this);
    }
    insert(document) {
        this._batchCounts.nInsertOps++;
        helpers_1.assertArgsDefinedType([document], [true], 'Bulk.insert');
